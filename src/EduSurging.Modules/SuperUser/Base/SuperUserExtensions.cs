...
namespace SuperUser.Base
{
    ...
    {
        public static void RegisterSuperUserModuleDbContext(this IServiceCollection services, IConfiguration Configuration)
        {
            services.AddDbContext<SuperUserDbContext>(optionsBuilder =>
            {
                var dbContextOptionsBuilder = optionsBuilder.UseLazyLoadingProxies();
                dbContextOptionsBuilder.UseMySql(Configuration.GetConnectionString("SuperUserConnection"), new MySqlServerVersion(new Version(8, 0, 21)));
            });
        }
